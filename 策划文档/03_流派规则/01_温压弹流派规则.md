...
```gdscript
final_impact_damage = base_impact_damage * chain_multiplier * positive_effect_muls
final_explosion_damage = base_explosion_damage * chain_multiplier * positive_effect_muls
final_projectile_count = base_projectile_count * chain_multiplier + scaled_positive_adds
final_explosion_radius = base_explosion_radius * scaled_positive_radius_muls
```
负面数值效果继承但不吃连锁倍率。
...
| 卡名 | 费用 | 卡牌文本 | 程序效果 |
|---|---:|---|---|
| 试射温压弹 | 0 | 本次连锁爆炸伤害 +20%。 | `chain_effect(thermobaric).explosion_damage_mul += 20%` |
| 压力校准弹 | 0 | 本次连锁爆炸范围 +20%。 | `chain_effect(thermobaric).explosion_radius_mul += 20%` |
