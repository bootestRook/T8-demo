...
| 卡名 | 费用 | 卡牌文本 | 程序效果 |
|---|---:|---|---|
| 电容试放 | 0 | 本次连锁伤害 +20%。 | `chain_effect(electro_pierce).pierce_damage_mul += 20%` |
| 磁极校准 | 0 | 本次连锁范围 +20%。 | `chain_effect(electro_pierce).explosion_radius_mul += 20%` |
...
| 电磁分流 | 2 | 额外释放 1 次，伤害 -20%。 | `release_count += 1; release_damage_mul=0.8; release_interval=0.18` |
...
| 电磁分流+ | 4 | 额外释放 1 次。 | `release_count += 1; release_damage_mul=1.0; release_interval=0.18` |
