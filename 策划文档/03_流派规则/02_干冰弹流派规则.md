...
```gdscript
final_damage = base_damage * chain_multiplier * positive_effect_muls
final_projectile_count = base_projectile_count * chain_multiplier + scaled_positive_adds
final_pierce = base_pierce * chain_multiplier + scaled_positive_adds
```
...
| 卡名 | 费用 | 卡牌文本 | 程序效果 |
|---|---:|---|---|
| 试射干冰弹 | 0 | 本次连锁伤害 +15%。 | `chain_effect(dry_ice).damage_mul += 15%` |
| 冷凝校准弹 | 0 | 本次连锁穿透 +1。 | `chain_effect(dry_ice).pierce_add += 1` |
| 散射小冰弹 | 1 | 首次命中后分裂为 3 个小冰弹。 | `split_on_first_hit={count=3, child_damage=45, child_pierce=1}` |
...
| 干冰弹连发 | 3 | 额外释放 1 枚，伤害 -20%。 | `release_count += 1; release_damage_mul=0.8; release_interval=0.18` |
