...
func _ready() -> void:
	...
	if _output_dir.is_empty():
		print("DiscardHandButtonScreenshotRunner loaded without --screenshot-dir; dry-load only.")
		get_tree().quit(0)
		return
...
func _run_capture_sequence(instance: Node) -> void:
	...
	if hud == null:
		push_error("Hud not found for discard hand button screenshot.")
		get_tree().quit(4)
		return
	hud.start_battle_ui()
	...
	_save_viewport("discard-hand-cooldown")
	get_tree().quit(0)
