...
const HAND_CARD_IDS := ["starter_infinite_fire", "starter_first_aid", "starter_temporary_shield"]
...
func _ready() -> void:
	...
	if _output_dir.is_empty():
		print("CardNoChargeOverlayScreenshotRunner loaded without --screenshot-dir; dry-load only.")
		get_tree().quit(0)
		return
...
func _run_capture_sequence(instance: Node) -> void:
	...
	if hud == null:
		push_error("Hud not found for no charge overlay screenshot.")
		get_tree().quit(4)
		return
	PrototypeState.hand_card_ids = HAND_CARD_IDS.duplicate()
	PrototypeState.current_energy = 1.0
	...
	_save_viewport("card-no-charge-overlay")
	get_tree().quit(0)
