...
const HAND_CARD_IDS := ["starter_infinite_fire", "starter_steady_aim", "starter_electromagnetic_pierce_wildcard", "starter_growth_plan"]
...
func _ready() -> void:
	...
	if _output_dir.is_empty():
		print("ModalOverlayConsistencyScreenshotRunner loaded without --screenshot-dir; dry-load only.")
		get_tree().quit(0)
		return
...
func _run_capture_sequence(instance: Node) -> void:
	...
	if hud == null:
		push_error("Hud not found for modal overlay consistency screenshot.")
		get_tree().quit(4)
		return
	_prepare_hand_state(hud)
	if hud.has_method("_open_pause_overlay"):
		hud.call("_open_pause_overlay")
	await _wait_frames(24)
	_save_viewport("pause-overlay")
	if hud.has_method("_close_pause_overlay"):
		hud.call("_close_pause_overlay")
	_prepare_reward_state(hud)
	await _wait_frames(24)
	_save_viewport("reward-overlay")
	get_tree().quit(0)


func _prepare_hand_state(hud: Hud) -> void:
...
func _prepare_reward_state(hud: Hud) -> void:
	...
	PrototypeState.upgrade_choices = [
		{
			"kind": "new_card",
			"type": "card",
			"title": "战术补给",
			...
			"cost_display": "1",
			"energy_cost": 1,
			"school_owned_count_text": "X4",
		},
		{
			"kind": "new_card",
			"type": "card",
			"title": "干冰补给",
			"description": "伤害-20%，从牌库抽2张干冰弹牌",
			"school": "干冰弹",
			"cost": 1,
			"cost_display": "1",
			"energy_cost": 1,
