...
| 路径 | 状态 | 作用 |
|---|---|---|
| `scenes/Game.tscn` | 存在 | 主场景节点结构，只保留战斗根节点、运行时动画节点和独立 UI 场景实例。 |
| `scenes/ui/Hud.tscn` | 存在 | 独立 HUD 稳定骨架场景，保留 CanvasLayer、Root、容器、关键标签/按钮/进度条和卡牌槽位。 |
| `scenes/ui/MainMenuScreen.tscn` | 存在 | 独立主界面场景，负责通关状态、关卡切换和开始入口。 |
| `scenes/Game.gd` | 存在 | 场景编排、输入、对象同步和表现层绘制顺序。 |
| `src/game/GameBattlefieldDrawer.gd` | 存在 | 战场背景与区域表现层绘制 helper，负责战场底图、墙体 fallback、占位底座和 active_areas 区域提示，不持有玩法状态。 |
| `src/game/GameCombatEffectDrawer.gd` | 存在 | 战斗效果表现层绘制 helper，负责 combat_effects 圆环、爆炸区域和电磁穿刺线光效，不持有玩法状态。 |
| `src/game/GameDamageFloatDrawer.gd` | 存在 | 伤害飘字表现层绘制 helper，负责 combat_effects 玩家伤害数字、暴击强调和元素颜色，不持有玩法状态。 |
| `src/game/GameProjectileDrawer.gd` | 存在 | 投射物表现层绘制 helper，负责 active_projectiles 的线段/弹体绘制和方向计算，不持有玩法状态。 |
| `src/game/PrototypeState.gd` | 存在 | 玩法状态 facade、public API、规则编排和统一信号入口；独立系统逻辑按 AGENTS 边界拆到专责模块。 |
| `src/game/CardConfigLoader.gd` | 存在 | 完整卡牌配置 JSON 读取入口，把 assets/data/cards/card_configs.json 转成运行时卡牌配置数组。 |
| `src/game/CardTextLoader.gd` | 存在 | 卡牌文本、费用和显示字段 CSV/JSON 覆盖入口，不承载完整卡牌规则表。 |
| `src/game/CardDeckState.gd` | 存在 | 手牌、牌堆、弃牌堆、补牌节奏和卡牌奖励落点运行时。 |
| `src/game/CardPlayRuntime.gd` | 存在 | 出牌运行时：出牌锁、同名冷却、复制牌记忆、pending bonus 和命中后奖励。 |
| `src/game/CardChainState.gd` | 存在 | 卡牌连锁状态、连续费用链推进和同流派前置效果继承。 |
| `src/game/CardChainRules.gd` | 存在 | 卡牌连锁效果表、基础倍率和正面数值缩放规则。 |
| `src/game/CardChainParamRules.gd` | 存在 | 卡牌连锁参数规范加载、实时文本数值格式化和 starter 运行时缩放 helper。 |
| `src/game/CoreSkillPayloadBuilder.gd` | 存在 | 核心技能 cast event、preview payload 和卡牌链加成 payload 构造。 |
| `src/game/StarterCardEffectResolver.gd` | 存在 | starter 卡效果解析，处理枪械 buff、治疗、护盾、标记、眩晕和成长牌。 |
| `src/game/GunRuntimeState.gd` | 存在 | 枪械弹药、换弹、开火计时、临时 buff、核心技能 runtime 和 shot log。 |
| `src/game/modules/combat/gun/GunEvents.gd` | 存在 | 枪械事件常量、注册入口和 route public API facade。 |
| `src/game/modules/combat/gun/GunRouteTable.gd` | 存在 | 枪械 cast/hit/resolved route 阶段组合。 |
| `src/game/modules/combat/gun/GunCastRoutes.gd` | 存在 | 枪械开火投射物生成 route。 |
| `src/game/modules/combat/gun/GunHitRoutes.gd` | 存在 | 枪械子弹命中、爆炸触发和分裂触发 hit route。 |
| `src/game/modules/combat/gun/GunResolvedRoutes.gd` | 存在 | 枪械爆炸查询/伤害和分裂子弹生成 resolved route。 |
| `src/game/CombatRuntime.gd` | 存在 | 战斗实体运行时 facade，负责 tick 编排、事件路由执行和实体数组状态维护。 |
| `src/game/CombatSnapshotBuilder.gd` | 存在 | 战斗运行时 snapshot 组装和深拷贝输出。 |
| `src/game/CombatMonsterCatalog.gd` | 存在 | 默认怪物规格和 ContentUnits 自定义怪物规格读取。 |
| `src/game/CombatTargetSelector.gd` | 存在 | 战斗目标选择、距离过滤、线段命中查询和目标排序。 |
| `src/game/CombatEffectLog.gd` | 存在 | 战斗效果显示队列、命令日志和上限裁剪。 |
| `src/game/CombatSpawnRuntime.gd` | 存在 | 刷怪组队列推进、批量生成、出生位置和波次数值系数。 |
| `src/game/CombatAreaRuntime.gd` | 存在 | 区域效果创建、生命周期 tick 和区域 tick 事件路由。 |
| `src/game/CombatProjectileRuntime.gd` | 存在 | 投射物延迟生成、飞行推进、命中事件路由和即时投射物表现。 |
| `src/game/CombatProjectileFactory.gd` | 存在 | 投射物 spawn delay、origin、target context 和 projectile 字典构造。 |
| `src/game/CombatDamageRuntime.gd` | 存在 | 伤害结算、状态应用、持续伤害 tick、元素判定和暴击判定。 |
| `src/game/CombatMonsterRuntime.gd` | 存在 | 怪物移动推进、到墙攻击、死亡经验、Boss 死亡标记和实体查找/索引。 |
| `src/game/CombatCommandPayload.gd` | 存在 | 战斗命令 target 数组标准化、爆炸中心和爆炸半径 payload 解析。 |
| `src/game/CombatCommandRuntime.gd` | 存在 | 战斗 query targets 服务和击退命令执行。 |
| `src/game/CombatUnitScale.gd` | 存在 | 战斗半径和投射物速度等单位换算公式。 |
| `src/game/modules/combat/skills/thermobaric/ThermobaricEvents.gd` | 存在 | 温压弹事件常量和 route public API facade。 |
| `src/game/modules/combat/skills/thermobaric/ThermobaricRouteTable.gd` | 存在 | 温压弹 cast/hit/resolved route 阶段组合。 |
| `src/game/modules/combat/skills/thermobaric/ThermobaricCastRoutes.gd` | 存在 | 温压弹 cast 阶段投射物生成 route。 |
| `src/game/modules/combat/skills/thermobaric/ThermobaricHitRoutes.gd` | 存在 | 温压弹冲击、爆炸、击退、燃烧、火花命中 hit route。 |
| `src/game/modules/combat/skills/thermobaric/ThermobaricResolvedRoutes.gd` | 存在 | 温压弹爆炸结束后火花投射物生成 resolved route。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceEvents.gd` | 存在 | 电磁穿刺事件常量和 route public API facade。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceRouteTable.gd` | 存在 | 电磁穿刺 cast/hit/resolved/tick route 阶段组合。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceCastRoutes.gd` | 存在 | 电磁穿刺 cast 阶段投射物生成 route。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceHitRoutes.gd` | 存在 | 电磁穿刺命中、麻痹、爆炸触发和裂变粒子 hit route。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceResolvedRoutes.gd` | 存在 | 电磁穿刺爆炸和矩阵区域生成 resolved route。 |
| `src/game/modules/combat/skills/electro_pierce/ElectroPierceTickRoutes.gd` | 存在 | 电磁矩阵 tick 查询、伤害和减速 route。 |
| `src/game/modules/combat/skills/dry_ice/DryIceEvents.gd` | 存在 | 干冰弹事件常量和 route public API facade。 |
| `src/game/modules/combat/skills/dry_ice/DryIceRouteTable.gd` | 存在 | 干冰弹 cast/hit/resolved route 阶段组合。 |
| `src/game/modules/combat/skills/dry_ice/DryIceCastRoutes.gd` | 存在 | 干冰弹 cast 阶段穿透投射物生成 route。 |
| `src/game/modules/combat/skills/dry_ice/DryIceHitRoutes.gd` | 存在 | 干冰弹命中伤害、击退、冻结、冻伤和小冰晶 hit route。 |
| `src/game/modules/combat/skills/dry_ice/DryIceResolvedRoutes.gd` | 存在 | 干冰弹首次命中分裂小冰晶 resolved route。 |
| `src/game/LevelRewardRuntime.gd` | 存在 | 升级三选一运行时、已选次数、升级应用和新卡获得事件。 |
| `src/game/WaveRuntimeState.gd` | 存在 | 波次配置、推进、spawn log、Boss 标记和战斗清场判断。 |
| `src/game/GameSnapshotBuilder.gd` | 存在 | 玩法运行时 snapshot facade，负责分段字段合并顺序和战斗 snapshot 合并入口。 |
| `src/game/GameSnapshotSections.gd` | 存在 | 玩法运行时 snapshot 分段字段组装 helper，负责核心、成长资源、卡牌、连锁和运行时日志字段。 |
| `src/game/UpgradePoolLoader.gd` | 存在 | 升级三选一奖励池 JSON/CSV 读取、短期默认池 fallback、权重、前置和互斥字段解析。 |
| `src/game/ContentUnits.gd` | 存在 | 关卡、波次、怪物规格和默认波次兜底的数据入口。 |
...
| `src/ui/HudSetupCoordinator.gd` | 存在 | HUD 初始化 wiring，负责控制器 setup、层级/鼠标过滤、信号连接、FX/overlay/presenter setup。 |
| `src/ui/HudBattleUiFlow.gd` | 存在 | HUD 战斗 UI 生命周期编排，负责开始/停止战斗 UI、暂停/继续/退出、阻塞 overlay 判断和统一 FX reset。 |
| `src/ui/HudDiscardHandFlow.gd` | 存在 | HUD 弃牌 UI flow，负责弃牌按钮响应、弃牌动画完成回调和阻塞文案拼装。 |
| `src/ui/HudHandQueries.gd` | 存在 | HUD 手牌几何和只读查询 helper，负责当前手牌数量、点位命中、panel 索引、拖拽重排目标和卡名查询。 |
| `src/ui/HudSnapshotPresenter.gd` | 存在 | HUD snapshot 基础状态字段呈现，负责时间、关卡、波次、经验、血量、护盾、弹药、能量和英雄名刷新。 |
| `src/ui/HudCardSnapshotPresenter.gd` | 存在 | HUD 手牌 snapshot 呈现，负责把手牌卡牌数据写入已有 card widget，不负责节点复制、布局或动效。 |
| `src/ui/HudPileOverlay.gd` | 存在 | 牌堆/弃牌堆弹窗 facade，负责显示状态、布局、卡牌重建和关闭输入。 |
| `src/ui/HudPileOverlayView.gd` | 存在 | 牌堆/弃牌堆弹窗稳定节点树 facade，负责遮罩、居中容器、面板和内容列编排。 |
| `src/ui/HudPileOverlayHeaderView.gd` | 存在 | 牌堆/弃牌堆弹窗标题栏、标题 label 和关闭按钮创建。 |
| `src/ui/HudPileOverlayContentView.gd` | 存在 | 牌堆/弃牌堆弹窗滚动区、卡牌网格容器和空状态 label 创建。 |
| `src/ui/HudCardInteraction.gd` | 存在 | HUD 手牌输入状态机，负责键盘/鼠标/拖拽/重排/取消的状态推进。 |
| `src/ui/HudCardInteractionPort.gd` | 存在 | HUD 手牌交互对 Hud.gd 的纯接口适配，集中几何查询、冷却查询、出牌/重排请求和提示反馈调用。 |
| `src/ui/HudCardVisuals.gd` | 存在 | HUD 手牌视觉生命周期 facade，只编排手牌可见性、feel state、目标变换、反馈叠加、按压 FX 和插值应用。 |
| `src/ui/HudCardVisualTarget.gd` | 存在 | HUD 手牌目标变换 facade，只编排槽位、运动和按压 helper。 |
| `src/ui/HudCardVisualTargetSlots.gd` | 存在 | HUD 手牌基础槽位、重排预览、悬浮扩散和边界夹取计算。 |
| `src/ui/HudCardVisualTargetMotion.gd` | 存在 | HUD 手牌拖拽、可出牌拖拽、悬浮和选中目标运动计算。 |
| `src/ui/HudCardVisualTargetPress.gd` | 存在 | HUD 手牌按压倾斜、位移和缩放目标变换。 |
| `src/ui/HudCardPressFeedback.gd` | 存在 | HUD 手牌悬浮按压命中区域、卡面点、抬升点和按压法线计算。 |
| `src/ui/HudCardVisualFeedback.gd` | 存在 | HUD 手牌无效、抽牌、获卡、弃牌和连锁费用提示反馈叠加。 |
| `src/ui/HudCardVisualInterpolator.gd` | 存在 | HUD 手牌目标位置、缩放、旋转和 z-index 的最终插值应用。 |
| `src/ui/HudCardWidgets.gd` | 存在 | HUD 卡牌控件创建、文本填充、费用状态和手牌排布入口。 |
| `src/ui/HudCardArtBinder.gd` | 存在 | HUD 卡牌卡框/卡图 TextureRect 创建、流派图标路径和纹理加载。 |
| `src/ui/HudCardLabelStyler.gd` | 存在 | HUD 卡牌标签样式 facade 和兼容缩放 API。 |
| `src/ui/HudCardLabelStyleGroups.gd` | 存在 | HUD 卡牌费用、插图、名称、类型、描述和流派标签字体、颜色、描边和 stylebox 分组应用。 |
| `src/ui/HudCardNodeFinder.gd` | 存在 | HUD 卡牌相关递归节点查找和 unique name 清理 helper。 |
| `src/ui/HudCardChainHintWidget.gd` | 存在 | HUD 连锁费用提示 facade，保留 widget 字典同步、费用徽章闪光和珍珠闪烁更新。 |
| `src/ui/HudCardChainHintNodes.gd` | 存在 | HUD 连锁费用提示 glow panel、sparkle label 查找/创建和基础样式。 |
| `src/ui/HudCardReadabilityRegions.gd` | 存在 | HUD 卡牌可读区布局 facade，只编排矩形数据、装饰层和文字区应用。 |
| `src/ui/HudCardReadabilityRegionFrames.gd` | 存在 | HUD 卡牌费用、名称、插图、描述、流派和连锁费用闪光挂点的纯矩形数据。 |
| `src/ui/HudCardReadabilityLayerApplier.gd` | 存在 | HUD 卡牌框、禁用遮罩、连锁费用闪光和按压反馈层布局应用。 |
| `src/ui/HudCardReadabilityTextApplier.gd` | 存在 | HUD 卡牌费用、名称、类型、插图、描述和流派文本区布局应用。 |
| `src/ui/HudCardDescriptionText.gd` | 存在 | HUD 卡牌描述 RichTextLabel 创建、旧 DescLabel 兼容隐藏和 BBCode 文本应用。 |
| `src/ui/HudCardHandLayout.gd` | 存在 | HUD 手牌排布、重叠间距、基础位置/旋转/z-index 和 snap 状态。 |
| `src/ui/HudLayoutStyler.gd` | 存在 | HUD 响应式布局、稳定区域尺寸、手牌布局触发和终极条锁图标。 |
| `src/ui/HudLayoutStyleApplier.gd` | 存在 | HUD 静态主题应用 facade，只编排面板、标签和控件样式 helper。 |
| `src/ui/HudLayoutPanelStyleApplier.gd` | 存在 | HUD 面板样式 facade，只保留兼容入口并委托分组 helper。 |
| `src/ui/HudLayoutPanelStyleGroups.gd` | 存在 | HUD 面板、徽章、手牌区域和菜单/暂停面板 stylebox 分组应用。 |
| `src/ui/HudLayoutLabelStyleApplier.gd` | 存在 | HUD 非卡牌标签样式 facade，只保留兼容入口并委托分组 helper。 |
| `src/ui/HudLayoutLabelStyleGroups.gd` | 存在 | HUD 非卡牌标签基础样式、主菜单、暂停、战斗状态、资源/护盾和目标标签分组样式应用。 |
| `src/ui/HudLayoutControlStyleApplier.gd` | 存在 | HUD 按钮、进度条和结算 overlay 样式应用。 |
| `src/ui/HudRewardOverlay.gd` | 存在 | 升级三选一奖励弹窗 facade，负责弹窗生命周期、按钮容器、输入和选择反馈。 |
| `src/ui/HudRewardOverlayView.gd` | 存在 | 升级三选一奖励弹窗稳定节点树 facade，只保留兼容入口并委托节点 helper。 |
| `src/ui/HudRewardOverlayViewNodes.gd` | 存在 | 升级三选一奖励弹窗遮罩、居中容器、面板、标题和选项容器节点创建。 |
| `src/ui/HudRewardChoiceButtons.gd` | 存在 | 升级三选一奖励选项按钮清空、创建、样式和 pressed 信号绑定。 |
| `src/ui/HudRewardChoiceContent.gd` | 存在 | 升级三选一奖励选项内容 facade，保留兼容入口并委托节点和文本 helper。 |
| `src/ui/HudRewardChoiceContentNodes.gd` | 存在 | 升级三选一奖励选项卡标题、描述、流派行和角标节点树构建。 |
| `src/ui/HudRewardChoiceText.gd` | 存在 | 升级三选一奖励选项标题、描述、费用/流派数量文本、类型标签和签名规则。 |
| `src/ui/HudRewardChoiceBadges.gd` | 存在 | 升级三选一奖励选项徽章 facade，保留费用、类型、同费用/同流派数量徽章兼容入口。 |
| `src/ui/HudRewardChoiceCostBadge.gd` | 存在 | 升级三选一奖励选项费用徽章和同费用数量 label 节点树与样式构建。 |
| `src/ui/HudRewardChoiceEntryTypeBadge.gd` | 存在 | 升级三选一奖励选项类型徽章的 holder、panel、margin 和 label 节点树与样式构建。 |
| `src/ui/HudRewardChoiceSchoolBadge.gd` | 存在 | 升级三选一奖励选项流派徽章、同流派数量 label 和流派行节点树与样式构建。 |
| `src/ui/MainMenuScreen.gd` | 存在 | 独立主界面逻辑，读取关卡配置并发出开始关卡信号。 |
| `src/ui/HudDrawCardFx.gd` | 存在 | 牌堆抽牌飞入手牌的计数同步、FX 队列状态、飞行动画更新、牌堆脉冲和到达反馈。 |
| `src/ui/HudDrawCardFxSpawn.gd` | 存在 | 牌堆抽牌飞行动效的目标校验、卡牌节点复制初始化、起点和 fx 数据构造。 |
| `src/ui/HudDiscardHandFx.gd` | 存在 | 整手弃牌飞入弃牌堆的生命周期、FX 队列状态、飞行动画更新、弃牌堆脉冲和完成回调。 |
| `src/ui/HudDiscardHandFxSpawn.gd` | 存在 | 整手弃牌飞行动效的目标校验、卡牌节点复制初始化和 fx 数据构造。 |
| `src/ui/HudPlayCardFx.gd` | 存在 | 出牌飞行动效生命周期、卡牌复制、飞入/停留/溶解阶段编排。 |
| `src/ui/HudPlayCardFxFragments.gd` | 存在 | 出牌溶解碎片节点创建、颜色、散开轨迹、碎片淡出和释放。 |
| `src/ui/HudAcquireCardFx.gd` | 存在 | 获得新卡进入手牌/牌堆的事件同步、FX 队列状态、飞行动画更新和到达反馈。 |
| `src/ui/HudAcquireCardFxSpawn.gd` | 存在 | 获得新卡飞行动效的事件解析、节点初始化、起终点/缩放/旋转和 fx 数据构造。 |
| `src/ui/HudAcquireCardFxCardBuilder.gd` | 存在 | 获得新卡飞行动效的卡牌节点复制、纹理、文本和样式填充。 |
...
| `scripts/encoding_review.py` | 存在 | UTF-8 文本解码、PowerShell 问号替换和 Excel CSV BOM 规则检查。 |
...
| `scripts/validate_data_configs.py` | 存在 | 关卡、波次、怪物和卡牌 JSON 配置结构及跨表引用校验。 |
| `scripts/card_chain_rule_check.gd` | 存在 | 卡牌连锁规则专项 headless 检查，覆盖连续费用、断链、继承和正负数值缩放。 |
...
| `scripts/package_playable.py` | 存在 | 生成 Web 本地可游玩包，包含启动游戏.cmd、本地服务脚本、www 和 zip。 |
| `scripts/package_windows.py` | 存在 | 生成原生 Windows 可游玩包，包含同名 exe、pck、README 和 zip。 |
